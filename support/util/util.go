...
func PredicatesForHostedClusterAnnotationScoping(r client.Reader) predicate.Predicate {
	fmt.Println("in PredicatesForHostedClusterAnnotationScoping")
	...
	filter := func(obj client.Object) bool {
		fmt.Println("in predicate filter")
		if hcAnnotationScopingEnabledEnvVal != "true" {
			fmt.Println("scoping is not enabled")
			return true
		}
...
func getHostedClusterScopeAnnotation(obj client.Object, r client.Reader) string {
	...
	case *hyperv1.HostedCluster:
		fmt.Println("in hostedcluster case")
	...
	default:
		...
		if nodePoolName != "" {
			...
			if err != nil {
				fmt.Println("unable to get nodepool")
				return ""
			}
			hostedClusterName = fmt.Sprintf("%s/%s", np.Namespace, np.Spec.ClusterName)
		}
	}
	fmt.Println("hostedclustername = ", hostedClusterName)
	...
	if err != nil {
		fmt.Println("unable to get hostedcluster")
		return ""
	}
